...
const DocsDeleteModal = ({show, setShow, data,refetch }) => {
    ...
    //     async (id) => {
    //          return await axiosPrivate.delete(`/documents/teacherDocs/${id}`);
    //      },
    //      {
    //          onSuccess: () => {
    //              refetch();
    ...
    //      }
    //  );
//"fields", "branches", "specialties", "courses"
     const deleteMutation = useMutation(
        () => {
            return axiosPrivate.delete(`/${data.selectedItem}/${data.id}`);
        },
        {
            onSuccess: () => {
                refetch();
    ...
    const handleDeleter = async() => {
        setDeleteStart(true)
        try{
        // await deleteFile(data?.subjectRef);
        // await deleteFile(data?.solutionRef);
...
<div className={` ${deleteStart? 'flex':'hidden'} absolute z-50 h-full  w-full bg-[#000000a5] right-0 top-0 rounded-lg  place-content-center place-items-center`}> <Loader/></div>
            ...
            <p className="mb-4 text-gray-500 dark:text-gray-300">Deleting this item will permanently remove it from the system, along with all its associated data. This action cannot be <span className='text-indigo-500'>undone.</span><br/> <span className='text-xl'>Are you sure you want to proceed?</span>
</p>
