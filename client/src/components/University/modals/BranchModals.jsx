...
export const CreateBranchModal = ({ show, setShow, refetch }) => {
...
export const UpdateBranchModal = ({ show, setShow, id, name, refetch }) => {
    const axiosPrivate = useAxiosPrivate();
    const [field, setField] = useState("");
    const [errMsg, setErrMsg] = useState("");
    ...
    const {
        ...
        [id],
        async () => {
            const response = await axiosPrivate.get(`/fields/${id}`);
            setField(response.data.name);
    ...
    const handleUpdater = (e) => {
        e.preventDefault();
        updateMutation.mutate({ id, name: field });
    };
    return (
        <>
            {/* <!-- Main modal --> */}
