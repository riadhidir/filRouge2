        ...
        <div className="flex gap-5 ">
          {/* <Link to={`/users/${data._id}`}
            ...
            <button
                    ...
                    onClick={() => edit({state:true, id:data?._id, course:data?.course, type: data?.type, language:data?.language, date:data?.date, answer:data?.answer, subject: data?.subject,cycle:data?.cycle,description:data?.description})}
                ...
                <button
                    onClick={() => remove({state:true, id:data._id, subjectRef:data?.subject?.ref, solutionRef:data?.answer?.ref})}
                    className="flex items-center gap-2 px-3 py-1.5 text-sm text-indigo-600 duration-150 bg-indigo-50 rounded-lg hover:bg-indigo-100 active:bg-indigo-200"
                >
