...
const DeleteAlldocsModal = ({
    ...
    const deleteMutation = useMutation(
        async (data) => {
            return await axiosPrivate.delete(`/documents/studentDocs`, {data});
        },
        {
            onSuccess: () => {
    ...
    const deleteHandler = async () => {
        ...
        try{
           ...
           await Promise.all(mainData.main.map(async(item) => {
                ...
                await deleteFile(item.link.ref) }
            }));
    ...
    return (
        <>
            ...
            {
                deleteStart &&  <div className="absolute bg-[#000000b8] w-full h-full right-0 flex place-content-center place-items-center">
                <Loader/>
            </div>
            }
